...
from kumiki.runtime import FeaturePack, PackStatus, install_command, install_runtime
...
class SetupSection(QWidget):
    ...
    def start(self) -> None:
        ...
        threading.Thread(target=run, name=f"kumiki-install-{self._pack.key}", daemon=True).start()
        self._timer.start()
