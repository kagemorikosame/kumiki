...
AI の操作は最終的に UI と同じ :class:`~novaedit.core.commands.Document` を通る。
ただし AI 層はウィジェットを知らないでいたいので、必要な操作だけをここに列挙して
おき、:class:`~novaedit.ui.main_window.MainWindow` がそれを満たす。
...
from novaedit.core.commands import Command, Document
from novaedit.core.model import ClipId, MediaId, MediaItem
...
class EditorHost(Protocol):
    ...
    ここのメソッドは**すべて UI スレッドで呼ばれる**（:mod:`novaedit.ai.bridge` が
