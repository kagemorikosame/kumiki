...
時計回り）は写す側（:mod:`kumiki.compat.ymm4.effects`）で直す
"""
...
from kumiki.effects.motion import _IN_OUT, _WAVE, _easing, _in_out_specs
...
def register_optics_effects() -> None:
    ...
    definitions = (
        ...
        EffectDefinition(
            ...
            # 絵は変えない 前のフレームを重ねるのはレンダラ（kumiki.engine.render.renderer）
