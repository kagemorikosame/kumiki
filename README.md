...
- エフェクト 13 種（色調補正・ぼかし・グロー・クロマキー・変形・クリッピング・縁取り・影・グラデーション・シャープ・ノイズ・モザイク・マスク）
...
- 配布テンプレートの棚。AviUtl2 のエイリアスと YMM4 のアイテムテンプレートを並べ、
...
このマシンに YMM4 が入っていないため、**YMM4 の対応付けは実ファイルでの
突き合わせができていません。** 形式に沿って書いてあり、振り分けは `$type` の
**クラス名だけ**で行うので名前空間やアセンブリ名が変わっても壊れませんが、
実際の `.ymmt` を 1 つ通せば、足りない対応が互換性レポートにそのまま並びます。
