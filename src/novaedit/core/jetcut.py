...
素材の無音は**ソース秒**で得られる（:mod:`novaedit.engine.audio.silence`）。それが
タイムラインのどこに当たるかは、その素材を使っているクリップごとに違う。ここは
字幕の投影（:mod:`novaedit.core.projection`）と同じ変換を、区間に対して行う。
...
from novaedit.core.model import Clip, MediaId, Project
from novaedit.core.timebase import FrameRate, Rounding, seconds_to_frame
