...
:class:`~novaedit.core.commands.Document` を直に触ると Qt が落ちるので、**やることを
キューへ積んで UI スレッドに実行させる**。

同じ仕組みで確認（承認）も運ぶ。変更系のツールは、UI 側が「許可」を返すまでここで
止まる。止まっている間も UI は動き続けるので、内容を見てから決められる。

:class:`~novaedit.engine.cache.MediaAnalyzer` や字幕起こしと同じ「キューに積んで
...
from novaedit.ai.operations import Operation
