...
class ObjApi:
    ...
    def _apply_segments(
        ...
        *,
        ...
        """``effects`` を頭から掛けられるだけ掛けた絵 掛けた分は ``effects`` から除く

        サイズ固定 のぼかしはそれだけで 1 回掛け、掛ける前の大きさへ戻す 前後の効果と
        まとめて掛けると、どこまでがぼかしで広がった分かが分からない 途中で掛けられなく
        なったら、残りは ``effects`` に積んだまま返す
        """
        while effects:
            count = _first_segment(effects)
            baked = self._baked(caller, image, tuple(effects[:count]), note=note)
            if baked is None:
                break
            image = baked
            del effects[:count]
    ...
    def _baked(
        ...
        *,
        ...
        """``image`` へ ``effects`` を 1 回で掛けた絵 掛けられなければ ``None``

        掛けられないときの効果は呼ぶ側が持ったままにする 捨てると効果ごと消える
        ``effects`` の最後が サイズ固定 のぼかしなら、それだけが渡される（:func:`_first_segment`）
        """
        ...
        baked: np.ndarray | None = apply(image, requests)
        if baked is not None and fixed:
            baked = _centre_crop(baked, image.shape[0], image.shape[1])
        return baked
