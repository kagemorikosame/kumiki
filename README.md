...
- エフェクト 49 種 標準の 17 種（色調補正・ぼかし・方向ぼかし・グロー・クロマキー・輝度キー・変形・クリッピング・縁取り・影・グラデーション・単色塗り・不透明度・シャープ・ノイズ・モザイク・マスク）に、YMM4 の配布物に合わせた動き 17 種（ランダム・反復・登場と退場・渦巻き・波・破片 ほか）と加工 15 種（角丸・露出・網点・グリッチ・長い影・輪郭抽出 ほか）
- 合成モード 8 種（通常・加算・減算・乗算・スクリーン・オーバーレイ・比較(明)・比較(暗)）と、板を傾ける X 軸・Y 軸の回転
...
AI に見せている操作は 42 種類（読み取り 15 / 変更 27）で、UI と同じ `Command` を通ります
