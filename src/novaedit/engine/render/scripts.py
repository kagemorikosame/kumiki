...
どう変形して描くか」の一覧（:class:`~novaedit.compat.aviutl.objapi.DrawCall`）に
なるので、レンダラはそれを順に合成すればよい。

**Lua は Qt を知らない。** テキストや図形を作る ``obj.load`` は
:func:`~novaedit.engine.sources.render_source` に委ねる。互換層をエンジンから
...
from novaedit.compat.aviutl import PREFIX
from novaedit.compat.aviutl.catalog import ScriptCatalog, script_catalog
from novaedit.compat.aviutl.control import lua_value
from novaedit.compat.aviutl.objapi import DrawCall, EffectRequest, ObjectState
from novaedit.compat.aviutl.runtime import LuaScriptRuntime
