...
確かめ、0 以外は YMM4 と一致した 0 は YMM4 が素材の頭の絵で止めるが、こちらには止めた絵を
表す仕組みが無いので、絵は等倍で動かし互換性レポートに数えて残す
...
| 枠 | YMM4 | Sashimono |
|---|---|---|
| `PlaybackRate` 100 | 1.00 倍 頭 0 一致 180 | 1.00 倍 頭 0 一致 120 |
| `PlaybackRate` 50 | 0.50 倍 頭 0 一致 180 | 0.50 倍 頭 0 一致 180 |
| `PlaybackRate` 200 | 2.00 倍 頭 0 一致 180 | 2.00 倍 頭 0 一致 60 |
| `PlaybackRate` 0 | 止まる 頭 0 一致 180 | 1.00 倍 頭 0 一致 120 |

0 以外の速さは一致した 0 は YMM4 が素材の頭の絵で止め、Sashimono は等倍で動く
（止めた絵を表す仕組みがまだ無い） 一致数の違いは速さとは別の話で、素材より枠が長いとき、
YMM4 は最後の絵で止めて出し続け、Sashimono は素材が終わると何も出さないことから来る
