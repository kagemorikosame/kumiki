...
書き換わったかは、更新時刻と大きさとファイルの番号で見る 中身を毎回読んで
比べると、読み直さないために毎回読むことになる
"""
...
from sashimono.engine.gpu.glutil import Texture

__all__ = ["EffectImages", "ImageTexture"]

_log = logging.getLogger(__name__)

#: ファイルの手がかり（更新時刻、大きさ、ファイルの番号） 無ければ ``None``
type _Signature = tuple[int, int, int] | None


class ImageTexture(Protocol):
    """GPU に置いた画像 :class:`~sashimono.engine.gpu.Texture` が満たす"""
...
class EffectImages:
    ...
    def release(self) -> None:
        ...
        # 伝えたことも忘れる 残すと、次に同じ画像が見つからなくなったときに黙る
        self._warned.clear()


def _signature(path: str) -> _Signature:
    """ファイルの手がかり（更新時刻、大きさ、ファイルの番号）

    番号（``st_ino`` Windows では NTFS のファイル ID）を入れるのは、別のファイルへ
    書いてから差し替える保存（多くの画像ソフトがそうする）で、更新時刻と大きさが
    元と同じでも気付くため 同じファイルを、時刻も大きさも変えずにその場で
    書き換えた場合は気付けない 中身を読んで比べると、1 秒おきの見張りで
    画像を毎回読むことになるので、そこは割り切っている
    """
    try:
