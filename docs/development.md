...
- 素材より長い動画アイテムは、置くときに素材を読んで長さが分かってから、
  ...
  PTS そのままの時刻で、`source_in` やデコーダが読む時刻と同じ数え方 コンテナの長さは
  頭の時刻を含まず、音の方が長い素材では最後の映像フレームより後ろも指すので混ぜない
  道の長さが分からない素材（前に取り込んだ物）はコンテナの長さで見る
  デコーダが絵を出すのも映像の道の終わりまでにした（道の長さが分からない素材はコンテナの
  頭の時刻 ＋ 長さ） 頭が 0 より後ろの素材（分割して書き出した物）は、それまで映像の
  ほとんどが何も映らなかった 音の方が長い素材は、それまで音だけの区間にも最後の絵が
