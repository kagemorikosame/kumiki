...
測った値（``tools/bench_proxy.py`` 3840x2160 95 パーセンタイル
**プレビューと同じ道**＝合成だけ 画面へ出さず CPU へ読み戻す分は数えない）

========================== =========== =======================
組み合わせ                   1 枚        3 枚重ね + blur/glow
========================== =========== =======================
元の素材 + 等倍              11.3ms      62.0ms
元の素材 + 1/2               11.4ms      59.5ms
控え 540p + 等倍              5.2ms      23.7ms
控え 540p + 1/2               5.1ms      14.8ms
========================== =========== =======================

1 枚だけなら元の素材でも入る 重ねると入らなくなり、**画面を落とすだけでは
ほとんど効かない**（62.0 → 59.5ms）のに対し、控えにすると 23.7ms まで落ちる
重ねた絵ではデコードが効いているため 予算に入れるには控えと
:class:`~kumiki.engine.render.RenderQuality` の両方が要る
