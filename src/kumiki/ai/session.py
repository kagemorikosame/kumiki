...
キュー越しにする（:mod:`kumiki.ai.bridge` と同じ考え方）
...
from kumiki.ai.environment import find_claude_cli
from kumiki.ai.server import SERVER_NAME, build_server

__all__ = ["SYSTEM_PROMPT", "AgentEvent", "AgentSession", "EventKind"]

#: エージェントへの指示 ツールの意味と、この編集ソフト特有の約束事を伝える
SYSTEM_PROMPT = """\
あなたは動画編集ソフト Kumiki の中で動く編集アシスタントです ユーザーの指示を、
...
class AgentSession:
    ...
    def start(self) -> None:
        if self.running:
            return
        self._closed.clear()
        self._thread = threading.Thread(target=self._run, name="kumiki-agent", daemon=True)
        self._thread.start()
