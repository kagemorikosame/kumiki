...
=================== ========= ========== ========== ========== ====================
1 枚                  11.2ms    4.9ms      4.7ms      4.6ms      元 + 等倍
2 枚                  21.0ms    8.8ms      8.3ms      8.5ms      控え + 等倍
3 枚                  32.0ms    13.5ms     13.3ms     13.2ms     控え + 等倍
3 枚 + blur           63.8ms    21.2ms     17.3ms     16.1ms     控え + 1/4
3 枚 + blur/glow      67.5ms    29.2ms     20.8ms     18.4ms     **どれも入らない**
=================== ========= ========== ========== ========== ====================

読み取れること

* **1 枚だけなら元の素材のままで入る** 重ねた時点で外れる（2 枚で 21.0ms）
* **効くのは控えの側** 3 枚で 32.0 → 13.5ms 画面を落とすだけでは 30.8ms で
  ほとんど効かない（重ねた絵ではデコードが重さの中心）
* **4K を 3 枚重ねて効果を積むと、控えだけでは足りない** blur だけなら
  控え + 1/4 まで落としてぎりぎり入る（16.1ms 予算まで 0.6ms しかない）
* **blur/glow まで積むと、どの組でも入らない**（一番速い控え + 1/4 で 18.4ms）
...
``tools\bench_proxy.py --width 3840 --height 2160 --layers 3 --effects blur --prefetch``
...
* 控えと組にすると貯めるのも速くなる（控え + 1/4 なら 1 枚 17.5ms）
* **貯めるのが現実的でない所もある** 1080p の動画を 20 本重ねて blur/glow を
  積むと 1 枚 2178.2ms で、3 秒ぶん（90 枚）貯めるのに 175 秒 出すのは 0.3ms の
  ままだが、先読みは万能ではない そこまで重ねる作り方は、素材を先に書き出して差し替える
...
MEASURED_THREE_LAYERS_MS = (32.0, 13.5, 13.2)
MEASURED_ONE_LAYER_MS = 11.2

#: 先読みの表（貯める 1 枚 / 出す 1 枚） 4K 3 枚 + blur を元の素材のまま
#: 設定の画面はここを指す
MEASURED_PREFETCH_MS = (65.1, 0.3)
