...
まだ読まない物（文字のまま残す 黙って捨てると描けていないことに気付けない）

- `<@+B>` `<@-B>` のスタイルの足し引き、`<r>` `<w>` `<c>` `<p>` `<gw>` `<gh>` `<tw>` `<th>`
  `<tr>` `<&>` `<$>` ふりがな・コメントの制御文字
- `<@メイリオ,3>` の文字装飾の番号と `<s>` のスタイル・縁取りの太さは、書体と大きさだけを
  読んで捨てる（`tag21` で縁取りが付くことは見た） 捨てた回数は互換性の記録に種類ごとに数える
- 本人が書き換えたプリセット色、大きさを変えた字の縁取りの太さ（今は設定欄の大きさで決まる）
- `<@書体>` の後の `<s>` がどの書体へ戻るか（設定欄か、`<s>` の前の書体か） 今は設定欄の書体へ戻すが、
  測ったのは `<@>` を挟まない `H<s80,Meiryo>H<s>H` だけ
