...
プレビューと同じ道＝合成と画面への転送 CPU へ読み戻す分は数えない
重ねる枚数は**別々の素材**として置く 同じ素材を重ねるとデコードが 1 回で済む）

=================== ========= ========== ========== ============== ===================
条件                  元 + 等倍  控え + 等倍  控え + 1/2  控え + 1/4     予算に入る一番きれいな組
=================== ========= ========== ========== ============== ===================
1 枚                  11.6ms    5.4ms      5.0ms      5.1ms        元 + 等倍
2 枚                  23.5ms    10.3ms     10.0ms     9.7ms        控え + 等倍
3 枚                  33.5ms    15.4ms     15.2ms     14.0ms       控え + 等倍
3 枚 + blur           68.9ms    21.0ms     17.7ms     16.6ms       控え + 1/4
3 枚 + blur/glow      71.5ms    32.5ms     22.4ms     19.6ms       **どれも入らない**
=================== ========= ========== ========== ============== ===================

読み取れること

* **1 枚だけなら元の素材のままで入る** 重ねた時点で外れる（2 枚で 23.5ms）
* **効くのは控えの側** 3 枚で 33.5 → 15.4ms 画面を落とすだけでは 33.4ms で
  ほとんど効かない（重ねた絵ではデコードが重さの中心）
* 効果を積むと控えだけでは足りず、画面の側も落として初めて入る（3 枚 + blur）
* **4K を 3 枚重ねて効果を 2 つ積むと、どの組でも入らない** ここから先は
  先読み（バックグラウンドレンダリング #29）の仕事
