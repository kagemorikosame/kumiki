...
from novaedit.core.model import Effect, GeneratedSource
...
class Preset:
    ...
    def from_dict(cls, data: object) -> Preset:
        ...
        if data.get("format") != FORMAT_NAME:
            raise ProjectFileError("NovaEdit のプリセットではない")
...
def default_preset_root() -> Path:
    ...
    if base:
        return Path(base) / "NovaEdit" / "presets"
    return Path.home() / ".config" / "novaedit" / "presets"
